...
| GHSA-59fj-m2j6-hcxh | high | Incomplete fix of CVE-2026-53925 (v4 `5c07c0d9`): the `--disable-config-exec` hardening covered AMP commands but not the on-alert action command lines read from the same `glances.conf`. v4 added `GlancesActions.allow_operators()` → `secure_popen(..., allow_operators=)`. v5 has **no** equivalent yet: `ShellAction` always runs `asyncio.create_subprocess_shell()` and `disable_config_exec` is absent from every `*_v5` module. The v5 actions/amps port must port an equivalent gate — when on, config-sourced action commands run through an explicit argument list (`shell=False`), never a shell. | Carry forward (AMP / actions port — **open gap**) |
